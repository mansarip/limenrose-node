function Designer() {
	...
	Designer.prototype.OpenConnectionWindow = function(selectItemId) {
		...
		toolbar.attachEvent('onClick', function(id){
			...
			if (id === '1') {
				...
				mode = 'add';
				layout.cells('b').attachHTMLString(designer.GetView('connectionAddNew'));
			}
		...
		$(layout.base).on('click', '.buttonPlaceholder input.test', function(){
			...
			if (detail.host === '') {
				...
				$.ajax({
					url:'/api/designer/testconnection',
					type:'post',
					data:detail,
					dataType:'json'
				})
		...
		$(layout.base).on('click', '.buttonPlaceholder input.reset', function(){
			var connectionForm = (mode === 'add') ? $('#connectionAddNew') : $('#connectionEdit');
			connectionForm.find('input:not(.test, .save, .reset), select').each(function(){
				$(this).val('');
			});
		...
		tree.attachEvent('onClick', function(id){
			...
			layout.cells('b').attachHTMLString(designer.GetView('connectionEdit'));
			$('#connectionEdit input:not(.save, .test), #connectionEdit select').each(function(){
				var key = $(this).attr('data-key');
				var value = designer.details.app.connection[id][key];
				$(this).val(value);
			});
		});
	...
	Designer.prototype.OpenDataSourceWindow = function(selectItemId) {
		...
		toolbar.attachEvent('onClick', function(id){
			...
			if (id === '1') {
				...
				mode = 'add';
				layout.cells('b').attachHTMLString(designer.GetView('dataSourceAddNew'));

				var dropdown = $('#dataSourceAddNew select.connection');
				for (var connName in designer.details.app.connection) {
					dropdown.append('<option value="'+ connName +'">'+ connName +'</option>');
				}
			}
	...
	Designer.prototype.GetView = function(id) {
		return '<div id="'+ id +'">'+ $('div[data-id="'+ id +'"]').html() +'</div>';
	};
